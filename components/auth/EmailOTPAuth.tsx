...
export const EmailOTPAuth: React.FC<EmailOTPAuthProps> = ({ onAuthSuccess, isAdmin = false, onSwitchToSignup }) => {
  ...
  const handleSendOTP = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err.userNotFound) {
        setError(err.message || (isAdmin ? 'No admin found with this email.' : 'No account found with this email. Please sign up first.'));
  ...
  const handleVerifyOTP = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isAdmin) {
        // Strict Role Check: Ensure the user actually has admin privileges
        if (data.role !== 'admin') {
    ...
    <div className="fixed inset-0 bg-black text-white flex flex-col items-center justify-center z-[100]" style={{ overflow: 'hidden', height: '100vh', top: '80px' }}>
      ...
      <div className="flex flex-col items-center justify-center w-full h-full" style={{ overflow: 'hidden' }}>
        ...
        <div className="w-full max-w-md mx-auto bg-black/60 backdrop-blur-xl border border-white/10 rounded-2xl p-4">
          ...
          {error && (
            <div className={`mb-2 p-1.5 rounded-lg text-xs ${error.includes('No account found') || error.includes('sign up')
          ...
          {step === 'email' ? (
            ...
            <form onSubmit={handleVerifyOTP} className="space-y-2">
              ...
              <button
                ...
                disabled={loading}
                className="w-full py-0.5 text-gold-400 hover:text-gold-300 text-xs transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              >
                Resend OTP
              </button>
              <button
