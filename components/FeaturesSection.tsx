      ...
      <div className="container-lg w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="features-grid grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 animate-fade-in-up" style={{ animationDelay: '0.2s' }}>
          {features.map((feature, index) => (
            <div 
              key={index} 
              className="features-grid-item bg-neutral-900 p-6 rounded-xl border border-white/10 transition-all hover:-translate-y-1.5 hover:border-indigo-500/30 hover:shadow-xl relative overflow-hidden group"
              style={{ animationDelay: `${0.1 * (index + 1)}s` }}
            >
              {/* Top gradient line on hover */}
              <div className="absolute top-0 left-0 right-0 h-0.5 bg-gradient-to-r from-indigo-500 to-rose-500 opacity-0 group-hover:opacity-100 transition-opacity"></div>
