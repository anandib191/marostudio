...
const replacements = [
  ...
  {r: /from-orange-500\s+via-rose-500\s+to-pink-500/g, s: 'from-gold-400 via-gold-500 to-gold-700'},
  {r: /from-indigo-400\s+via-indigo-500\s+to-rose-500/g, s: 'from-gold-400 via-gold-500 to-gold-700'},
  {r: /from-indigo-600\s+to-purple-600/g, s: 'from-gold-500 to-gold-700'},
];

function shouldSkip(dir) {
  return EXCLUDE.some(ex => dir.includes(path.sep + ex + path.sep) || dir.endsWith(path.sep + ex));
}
...
console.log(`Codemod complete. Files changed: ${changedFiles.length}. Total token replacements (approx): ${totalChanged}`);
if (changedFiles.length) console.log(changedFiles.map(f => `${f.file} (${f.replacements})`).join('\n'));
